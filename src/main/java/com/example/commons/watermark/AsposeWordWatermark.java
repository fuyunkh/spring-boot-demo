...
public class AsposeWordWatermark implements BaseWatermark {
    ...
    private int numPerColumn = 8;                            //每列多少个水印
        ...
        for (Section sect : doc.getSections()) {
            ...
            for (int i = 0; i < numPerLine; i++) {
                for (int k = 0; k < numPerColumn; k++) {
                    watermarkPara.appendChild(getShape(doc, watermarkText, left, top));
                    top = top + gapOfColumn + height;
                }
                top = topMargin;
//                System.out.println(i);
    ...
    private void addOneWatermarkText(Document doc, String watermarkText) throws Exception {
        // Insert the watermark into all headers of each document section.
        Paragraph watermarkPara = new Paragraph(doc);
...
//        watermark.getTextPath().setFitPath(false);
    ...
    public void addWatermark(String src, String target, String watermarkText, boolean isOne) throws Exception {
        ...
        if (isOne) {
            addOneWatermarkText(doc, watermarkText);
        } else {
            //平铺
